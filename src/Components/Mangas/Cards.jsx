...
function Card({category,name,image,id,autor,description}){
    let hexColor
                   switch(category){
                       case "Shonen":
                           hexColor="#F472B6"
    ...
    function manga(id){
        window.location.href=`/manga?category=${category}&name=${name}&image=${image}&autor=${autor}&mangaId=${id}&descp=${description}`
